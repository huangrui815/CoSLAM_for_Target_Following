			...
			if (mpt->pFeatures[camId]) {
				for (FeaturePoint* fp = mpt->pFeatures[camId]; fp->nextFrame;
					...
					mpt->nccBlks[camId].computeScaled(
							...
							pCoSLAM->slam[camId].m_smallScale, fp->xo, fp->yo);

					int x = max(0, min((int) fp->xo, pCoSLAM->slam[camId].m_img.w));
					int y = max(0, min((int) fp->yo, pCoSLAM->slam[camId].m_img.h));
...
//	getNCCBlocks(img2, corners2, nccblks2, blockScale);
	...
	ImgG& smallImg1 = m_smallImg[cam1];
	ImgG& smallImg2 = m_smallImg[cam2];

	PtrVec<NCCBlock> nccblks1, nccblks2;
	getScaledNCCBlocks(smallImg1, blockScale, corners1, nccblks1);
	getScaledNCCBlocks(smallImg2, blockScale, corners2, nccblks2);
