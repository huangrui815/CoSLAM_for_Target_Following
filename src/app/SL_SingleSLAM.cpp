...
int SingleSLAM::sendInitFeaturePoints(){
	...
	Mat_d& feats = m_tracker.klt_._featPts;
	Mat_i& flag = m_tracker.klt_._flag;

	for (int i = 0; i < feats.m; i++){
		initFeats.points.push_back(feats[2*i]);
		initFeats.points.push_back(feats[2*i+1]);
		initFeats.flags.push_back(flag[i]);
	}
	MyApp::pub_features[camId].publish(initFeats);
	return initFeats.points.size();
}
...
bool SingleSLAM::grabReadFrame() {
	if(!videoReader->grabFrame())
		return false;

	_ts = videoReader->getTimeStamp();
	//videoReader->getCurGrayImage(m_img.data);
	videoReader->readCurFrame(m_rgb.data, m_img.data);
	videoReader->readCurFrame(m_rgb.data, m_img_draw.data);
...
int SingleSLAM::receiveFeaturePoints(double ts, vector<unsigned int>& featureId,
		vector<float>& fts, uchar* smallImg){
	_ts = ts;
	if (smallImg)
		memcpy(m_smallImg.data, smallImg, m_smallImg.cols * m_smallImg.rows);

	return m_tracker.next(m_featPts, featureId, fts);
}
...
#include <sys/types.h>
//#define DEBUG_MODE
bool SingleSLAM::poseUpdate3D(const double* R0, const double* t0,
...
KeyPose* SingleSLAM::addKeyPose(bool bSelfMotion) {
	...
	pose->setThumbImage(m_smallImg, maxThumbW);
		...
		if (err1 < maxEpiErr && err2 < maxEpiErr) {
			...
			if (err1 < maxEpiErr && err2 < maxEpiErr) {
				...
				pM->nccBlks[camId].computeScaled(m_lastKeyPos->imgSmall,
						m_lastKeyPos->imgScale, cur_fp->xo, cur_fp->yo);

				int x = max(0, min((int) cur_fp->xo, m_rgb.w - 1));
				int y = max(0, min((int) cur_fp->yo, m_rgb.h - 1));
...
void SingleSLAM::getDistortedCoord(const Mat_d& pts_in, Mat_d& pts_out) {
	assert(pts_in.m > 0);
	pts_out.resize(pts_in.m, 2);

	for (int i = 0; i < pts_in.m; i++) {
		double nm[2];
		normPoint(iK, pts_in.data + 2 * i, nm);
		imagePoint(K.data, k_c.data, nm, pts_out.data + 2 * i);
	}
}
