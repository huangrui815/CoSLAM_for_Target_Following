...
void InitMap::detectSiftFeats(std::vector<cv::KeyPoint>& keypoints, cv::Mat& desc, int iCam) {
	  const ImgG* img = m_pImgGray[iCam];
	  cv::Mat frame(img->rows, img->cols, CV_8UC1, img->data);
	  int minHessian = 400;
	  ...
	  Mat_d& pts = m_siftPts[iCam];
	  Mat_d& ptsNorm = m_siftPts_undist[iCam];
	  pts.resize(num, 2);
 	  ptsNorm.resize(num, 2);

	  pts.resize(keypoints.size(), 2);
	  for (int i = 0; i < num; i++) {
	  		pts.data[2 * i] = keypoints[i].pt.x;
	  		pts.data[2 * i + 1] = keypoints[i].pt.y;
	  		double in[2], out[2];
	  		in[0] = pts.data[2 * i];
	  		in[1] = pts.data[2 * i + 1];
	  		undistorPointNew(K[iCam],kd[iCam],in,out);
	  		ptsNorm[2 * i] = out[0];
	  		ptsNorm[2 * i + 1] = out[1];
	  }

	  printf("Number of detected sift points in camera %d: %d \n", iCam, num);
}
...
void InitMap::detectSurfFeats(int iCam) {
	...
	int num = keyPts.size();
	printf("detectSurfFeats iCam %d keyPts.size(): %d\n", iCam, num);
...
//	ImgRGB tempImg;
//	tempImg.resize(m_pImgRGB[iCam]->w, m_pImgRGB[iCam]->h);
//	memcpy(tempImg.data, m_pImgRGB[iCam]->data,
//			tempImg.w * tempImg.h * sizeof(uchar));
//
//	drawCorners(tempImg, pts);
//	char wndName[256];
//	sprintf(wndName, "Camera %d", iCam);
//	imshow(wndName, tempImg);
//	cv::waitKey(-1);
}
...
void InitMap::detectCorners(int iCam) {
	...
	int IH = img->h;
	GPUKLT klt;
	klt.setIntrinsicParam(K[iCam], invK[iCam], invkd[iCam], kd[iCam]);
	...
	printf("detectCorners iCam %d keyPts.size(): %d\n", iCam, num);
...
void InitMap::detectCorners_CPU(int iCam) {
	const ImgG* img = m_pImgGray[iCam];
	//	KLT corner detector
	int IW = img->w;
	int IH = img->h;
	...
	Mat_d& pts = m_corners[iCam];
	Mat_d& ptsNorm = m_normCorners[iCam];
	int num = klt.points[0].size();
	printf("detectCorners iCam %d keyPts.size(): %d\n", iCam, num);

	pts.resize(num, 2);
	ptsNorm.resize(num, 2);
	vector<cv::Point2f>& corners = klt.points[0];

	for (int i = 0; i < num; i++) {
		pts.data[2 * i] = corners[i].x;
		pts.data[2 * i + 1] = corners[i].y;
		double in[2], out[2];
		in[0] = pts.data[2 * i];
		in[1] = pts.data[2 * i + 1];
		// remove the distortion
...
int InitMap::estimateFmat(const Mat_d& pts1, const Mat_d& pts2, Mat_d& F,
	...
	if (!findFMatrix(pts1, pts2, F, maxEpiErr, FF_RANSAC))
		return -1;
...
bool InitMap::apply(int frame0, std::vector<FeaturePoints*>& pFeaturePts,
		MapPointList& mapPts) {
	if (numCams < 2){
		printf("StereoMatch - at least two cameras are required!\n");
		return false;
	}
	...
	if (nSurfPts <= SLAM_FEATURE_WIDTH * SLAM_FEATURE_HEIGHT / 2) {
		nCorners = reconstructTracks(m_cornerTracks, m_numCornerTracks, 2,
				frame0, pFeaturePts, mapPts, 30.0);
		logInfo("number of corners reconstructed : %d\n", nCorners);
	}

	int numOutliers = removeOutliers(mapPts, 12.0);
...
bool InitMap::apply_new(int frame0, std::vector<FeaturePoints*>& pFeaturePts,
		MapPointList& mapPts) {
	if (numCams < 2){
		printf("StereoMatch - at least two cameras are required!\n");
		return false;
	...
	keypoints.resize(numCams);
	desc.resize(numCams);

	//detect surf features at all cameras
	for (int i = 0; i < numCams; i++){
		detectSiftFeats(keypoints[i], desc[i], i);

//		char filename[100];
//				sprintf(filename, "sift_%d.txt", i);
...
//		fclose(file);
	...
	//compute extrinsic parameters for all cameras
	if(!computeExtrinsic_5pts()){
		return false;
	}
	//reconstruct the initial map points
	int nSurfPts = reconstructTracks(m_surfTracks, m_numSurfTracks, 2, frame0,
			pFeaturePts, mapPts, 5.0);

	logInfo("number of SURF feature points reconstructed : %d\n", nSurfPts);

	int numOutliers = removeOutliers(mapPts, 3.0);
	logInfo("removed outliers:%d\n", numOutliers);
	if(numOutliers < 0)
		return false;
...
bool InitMap::apply_new_corner(int frame0, std::vector<FeaturePoints*>& pFeaturePts,
		MapPointList& mapPts) {
	if (numCams < 2){
		printf("StereoMatch - at least two cameras are required!\n");
		return false;
...
//			matchCornerNCCBetween(i, j, 0.6, 7.0, 60);
	...
	int nCorners = 0;
	if (nSurfPts < KLT_MAX_FEATURE_NUM) {
		nCorners = reconstructTracks(m_cornerTracks, m_numCornerTracks, 2,
				frame0, pFeaturePts, mapPts, 5.0);
		logInfo("number of corners reconstructed : %d\n", nCorners);
	}
...
bool InitMap::matchSurfBetween(int iCam, int jCam, double maxEpiErr) {
	Matching surfMatches;
	//matchSurf(m_surfDesc[iCam], m_surfDesc[jCam], surfMatches, 0.83);
	matchSurf(m_surfDesc[iCam], m_surfDesc[jCam], surfMatches, 0.7);

	Mat_d pts1, pts2;
	getMatchedPts(surfMatches, m_surfPts[iCam], m_surfPts[jCam], pts1, pts2);
	printf("SURF matches(%d-%d): %d\n", iCam, jCam, pts1.rows);

//#define DEBUG_MODE
#ifdef DEBUG_MODE
	ImgRGB outImg;
	//drawMatchesScale(*imgGray[iCam], pts1, *imgGray[jCam], pts2, outImg, 1.0);
	drawMatching(*m_pImgGray[iCam], pts1, *m_pImgGray[jCam], pts2, outImg, 1.0, 0);
	char wndName[256];
	sprintf(wndName, "SURF matches: %d-%d", iCam, jCam);
...
#ifdef DEBUG_MODE
	//drawMatching(*m_pImgGray[iCam], pts1, *m_pImgGray[jCam], pts2, outImg, 0.5, lFlag);
	drawMatching(*m_pImgGray[iCam], m_surfPts[iCam], *m_pImgGray[jCam], m_surfPts[jCam], m_surfMatches[iCam][jCam], outImg);
	sprintf(wndName, "SURF matches (inliers) : %d-%d", iCam, jCam);
	imshow(wndName, outImg);
	cv::waitKey(-1);
#endif
...
void InitMap::matchCornerNCCBetween(int iCam, int jCam, double minNcc,
	...
	getMatchedPts(m_surfMatches[iCam][jCam], m_siftPts[iCam], m_siftPts[jCam],
			seedPts1, seedPts2);
...
int InitMap::matchSiftBetween(int iCam, int jCam){
	cv::FlannBasedMatcher matcher;
	vector< vector< cv::DMatch >  > matches;
	matcher.knnMatch( desc[iCam], desc[jCam], matches, 2);
...
//	  waitKey(0);
	  ...
	  m_surfMatches[iCam][jCam].clear();
	  m_surfMatches[iCam][jCam].reserve(good_matches.size());
	  ...
	  //		fclose(file);
	  	for (int i = 0; i < good_matches.size(); i++) {
	  		m_surfMatches[iCam][jCam].add(good_matches[i].queryIdx,
	  					good_matches[i].trainIdx, good_matches[i].distance);
			fprintf(file, "%d %d %f\n", good_matches[i].queryIdx,
  					good_matches[i].trainIdx, good_matches[i].distance);
	  }
...
void InitMap::getSiftTracks() {
	...
	for (int i = 1; i < numCams; i++) {
		int iCam = camOrder.data[i - 1];
		int jCam = camOrder.data[i];
		_surfMatch2Ind(iCam, jCam, m_siftPts[iCam].rows, matchingIdx[i - 1]);
	}

	//the flag records the track id corresponding to this feature point
	for (int i = 0; i < numCams; i++) {
		int iCam = camOrder.data[i];
		flag[iCam].resize(m_siftPts[iCam].rows, 1);
		flag[iCam].fill(-1);
	}
	...
	for (int i = 0; i < numCams - 1; i++) {
		...
		int numPts = m_siftPts[iCam].rows;
		if (i == 0) {
			for (int n = 0; n < numPts; n++) {
				int m = matchingIdx[i].data[n];
				if (m >= 0) {
					double x1 = m_siftPts_undist[iCam].data[2 * n];
					double y1 = m_siftPts_undist[iCam].data[2 * n + 1];
					double x2 = m_siftPts_undist[jCam].data[2 * m];
					double y2 = m_siftPts_undist[jCam].data[2 * m + 1];

					double xo1 = m_siftPts[iCam].data[2 * n];
					double yo1 = m_siftPts[iCam].data[2 * n + 1];
					double xo2 = m_siftPts[jCam].data[2 * m];
					double yo2 = m_siftPts[jCam].data[2 * m + 1];

					m_surfTracks[m_numSurfTracks].add(i, x1, y1, xo1, yo1);
					m_surfTracks[m_numSurfTracks].add(i + 1, x2, y2, xo2, yo2);
			...
			for (int n = 0; n < numPts; n++) {
				if (flag[iCam].data[n] >= 0) { //existing track
					...
					if (m >= 0) { //extend the track
						...
						m_surfTracks[flag[iCam].data[n]].add(i + 1, x2, y2, xo2, yo2);
						flag[jCam].data[m] = flag[iCam].data[n];
					}
	...
	printf("Number of sift tracks found: %d \n", m_numSurfTracks);
	//	//test
	//	for (int i = 0; i < m_numSurfTracks; i++) {
	//		print(m_surfTracks[i]);
	//	}
...
bool InitMap::computeExtrinsic() {
	if (m_numCornerTracks <= 0){
		printf("MapInitializer::updateEMats - no feature track can be found");
		return false;
	}
...
//#define DEBUG_MODE
	...
	for (int i = 0; i < numCams; i++) {
		...
		for (int k = 0; k < mapPts.rows; k++) {
			double rm[2];
			project(K[iCam], camR[iCam], camT[iCam], mapPts.data + 3 * k, rm);
			//red -- reprojected point
			drawPoint(tmpImg, rm[0], rm[1], 3, 255, 0, 0);
			drawLine(tmpImg, rm[0], rm[1], m.data[2 * k], m.data[2 * k + 1], 255, 0, 0);
		}
...
bool InitMap::computeExtrinsic_new() {
	//use the camera pair that has the largest inter-camera distance
	int firstCam = camOrder[0];
	int lastCam = camOrder[numCams - 1];
...
bool InitMap::computeExtrinsic_5pts() {
	//use the camera pair that has the largest inter-camera distance
	int firstCam = camOrder[0];
	int lastCam = camOrder[numCams - 1];
	...
	CalibTwoCam calib;
	calib.setIntrinParam(K[firstCam], K[lastCam]);
	calib.setMatchedPoints(pts1, pts2);
	calib.estimateEMat(6.0);
	...
	Mat_d inPts1Norm, inPts2Norm;
	calib.outputInlierNormPoints(inPts1Norm, inPts2Norm);
	calib.outputRTs(tR0, tT0, tR1, tT1);

	int inNumPts = inPts1Norm.m;
	...
	inlierFlag.resize(pts1.rows, 1);
	inlierFlag.fill(0);
	for (int i = 0; i < inlierInd.size(); i++){
		inlierFlag.data[inlierInd[i]] = 1;
	}
...
int InitMap::reconstructTracks(const Track2D tracks[], int numTracks,
	...
	for (int k = 0; k < numTracks; k++) {
		...
		if (tracks[k].length() < minTrackLen)
			printf("Track length %d is smaller than minTrackLen %d\n",
					tracks[k].length(), minTrackLen);
			...
			if (!outlier && num < SLAM_FEATURE_HEIGHT * SLAM_FEATURE_WIDTH) {
				...
				for (Track2DNode* pTkNode = tracks[k].head.next; pTkNode;
					...
					FeaturePoint* pm = pFeaturePts[iCam]->add(frame0, iCam,
							pTkNode->xo, pTkNode->yo, pTkNode->x, pTkNode->y);
					...
					if (pTkNode == tracks[k].head.next) {
						...
						pM->setColor((*m_pImgRGB[iCam])(x,y));
					}
				}
				num++;
			}
...
int InitMap::removeOutliers(MapPointList& mapPts, double errMax) {
	...
	if (!pHead){
		printf("no map points is reconstructed!\n");
		return -1;
	}
