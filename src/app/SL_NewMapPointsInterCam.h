...
public:
	...
	void addSlam(SingleSLAM& sl, int curFrame) {
		...
		cloneImg(sl.m_smallImg, m_smallImg[numCams]);
		...
		for (int n = 0; n < fNum; n++) {
			...
			if (vecFeatPts[n]->mpt == 0 || vecFeatPts[n]->mpt->isFalse()) {
				...
				matFeatPts[numCams].data[2 * nFpts] = vecFeatPts[n]->xo;
				matFeatPts[numCams].data[2 * nFpts + 1] = vecFeatPts[n]->yo;
				nFpts++;
			}
		}
