...
void GLImagePane::drawFeaturePoints() {
	...
	// for debug
	int numOfStaticPts = 0;
	int numOfDynamicPts = 0;
			...
			if (b_drawReprojectionError) {
				...
				if (m_pCamPos)
					project(m_pSLAM->slam[m_camId].K, m_pCamPos->R, m_pCamPos->t, M,
							m);
				float x01 = static_cast<float>(m[0]);
				float y01 = static_cast<float>(m[1]);
				float x1, y1;
				...
				float reprojErr = (x - x1) * (x - x1) + (y - y1) * (y - y1);
				reprojErr = sqrt(reprojErr);
				if (p->mpt->isCertainStatic()){
					totalReprojErrStatic += reprojErr;
					numOfStaticPts++;
				} else if (p->mpt->isCertainDynamic()){
					totalreprojErrDynamic += reprojErr;
					numOfDynamicPts++;
				}
			}
		}
...
void GLImagePane::copyDisplayData() {
	...
	//copy the image
	memcpy(imgData, m_pSLAM->slam[m_camId].m_img_draw.data,
...
//	glEnd();
	...
	if (m_pSLAM->state[m_camId] == SLAM_STATE_RELOCATE) {
