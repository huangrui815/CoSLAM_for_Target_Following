...
unsigned int const extPixelFormat[] = { 0, GL_LUMINANCE, GL_LUMINANCE_ALPHA,
		GL_RGB, GL_RGBA };

inline int intFromString(std::string const& value, int defaultVal) {
	if (value.empty())
		return defaultVal;
...
template<typename T>
inline void interleavePixels(int const w, int const h, T const * red,
		T const * green, T const * blue, T * pixels) {
	for (int p = 0; p < w * h; ++p) {
		pixels[3 * p + 0] = red[p];
...
template<typename T>
inline void interleavePixels(int const w, int const h, T const * red,
		T const * green, T const * blue, T const * alpha, T * pixels) {
	for (int p = 0; p < w * h; ++p) {
		pixels[4 * p + 0] = red[p];
...
TextureSpecification::TextureSpecification(char const * specString) :
		nChannels(3), nBitsPerChannel(8), isFloatTexture(false), isDepthTexture(
				false), isRTT(true), enableTextureRG(false) {
	...
	while (!is.eof()) {
		...
		if (key == "r") {
			...
			this->nBitsPerChannel = intFromString(value,
					this->isFloatTexture ? 32 : 8);
		} else if (key == "rg") {
			this->nChannels = 2;
			if (value.find("f") != value.npos)
				this->isFloatTexture = true;
			...
			cerr << "TextureSpecification::TextureSpecification(): "
					<< "Warning Unknown keyword: '" << key << "'; ignored."
					<< endl;
		}
	} // end while
	...
	if ((this->nChannels < 3) && this->isRTT && !this->enableTextureRG) {
		cerr
				...
				<< "Warning: luminance or luminance/alpha texture will not work as render target."
				<< endl;
	}
} // end TextureSpecification::TextureSpecification()

unsigned int TextureSpecification::getGLInternalFormat() const {
	if (!this->isDepthTexture) {
		if (!this->enableTextureRG) {
			if (!this->isFloatTexture) {
				if (this->nBitsPerChannel == 8) {
					unsigned int const formats[5] = { 0, GL_LUMINANCE8,
							GL_LUMINANCE8_ALPHA8, GL_RGB8, GL_RGBA8 };
					return formats[this->nChannels];
				} else if (this->nBitsPerChannel == 16) {
					unsigned int const formats[5] = { 0, GL_LUMINANCE16,
							GL_LUMINANCE16_ALPHA16, GL_RGB16, GL_RGBA16 };
					return formats[this->nChannels];
				} else {
					raiseGLErrorHere1(
							"Unsupported number of bits for int texture (8 or 16 bits).");
				}
			} else {
				if (this->nBitsPerChannel == 32) {
					unsigned int const formats[5] = { 0, GL_LUMINANCE32F_ARB,
							GL_LUMINANCE_ALPHA32F_ARB, GL_RGB32F_ARB,
							GL_RGBA32F_ARB };
					return formats[this->nChannels];
				} else if (this->nBitsPerChannel == 16) {
					unsigned int const formats[5] = { 0, GL_LUMINANCE16F_ARB,
							GL_LUMINANCE_ALPHA16F_ARB, GL_RGB16F_ARB,
							GL_RGBA16F_ARB };
					return formats[this->nChannels];
				} else {
					raiseGLErrorHere1(
							"Unsupported number of bits for float texture (16 or 32 bits).");
				}
			} // end if (!useFloat)
		} else {
#if defined(GL_ARB_texture_rg)
			...
			if (!this->isFloatTexture) {
				if (this->nBitsPerChannel == 8) {
					unsigned int const formats[5] = { 0, GL_R8, GL_RG8, GL_RGB8,
							GL_RGBA8 };
					return formats[this->nChannels];
				} else if (this->nBitsPerChannel == 16) {
					unsigned int const formats[5] = { 0, GL_R16, GL_RG16,
							GL_RGB16, GL_RGBA16 };
					return formats[this->nChannels];
				} else {
					raiseGLErrorHere1(
				...
				if (this->nBitsPerChannel == 32) {
					unsigned int const formats[5] = { 0, GL_R32F, GL_RG32F,
							GL_RGB32F_ARB, GL_RGBA32F_ARB };
					return formats[this->nChannels];
				} else if (this->nBitsPerChannel == 16) {
					unsigned int const formats[5] = { 0, GL_R16F, GL_RG16F,
		...
		default:
			raiseGLErrorHere1(
					"Unsupported number of bits for depth texture (16, 24 or 32 bits).");
		}
...
bool ImageTexture2D::allocateID() {
...
void ImageTexture2D::overwriteWith(uchar const * pixels, int nChannels) {
...
void ImageTexture2D::overwriteWith(float const * redPixels,
		float const * greenPixels, float const * bluePixels) {
	unsigned const format = extPixelFormat[3];
	unsigned const type = GL_FLOAT;
...
void ImageTexture2D::clear() {
	glBindTexture(_textureTarget, _textureID);
	glTexImage2D(_textureTarget, 0, GL_RGB, 0, 0, 0, GL_RGB, GL_UNSIGNED_BYTE,
			0);
	checkGLErrorsHere1(_texName);
}
...
void ImageTexture2D::bind(unsigned texUnit) {
	glActiveTexture(texUnit);
	this->bind();
}

void ImageTexture2D::enable() {
	glBindTexture(_textureTarget, _textureID);
	glTexEnvi(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_REPLACE);
	glEnable(_textureTarget);
}

void ImageTexture2D::enable(unsigned texUnit) {
	glActiveTexture(texUnit);
	this->enable();
}

void ImageTexture2D::disable() {
	glDisable(_textureTarget);
}

void ImageTexture2D::disable(unsigned texUnit) {
	glActiveTexture(texUnit);
	this->disable();
}
...
bool FrameBufferObject::allocate() {
	glGenFramebuffersEXT(1, &_fboID);
	checkGLErrorsHere1(_fboName);
	return true;
}

void FrameBufferObject::deallocate() {
	std::fill(_attachedColorTextures, _attachedColorTextures + 16,
			(ImageTexture2D *) 0);
...
void FrameBufferObject::activate(bool setViewport) {
	this->makeCurrent();
...
bool FrameBufferObject::isCurrent() {
	GLint curFboID;
	glGetIntegerv(GL_FRAMEBUFFER_BINDING_EXT, &curFboID);
	return (_fboID == (GLuint) curFboID);
}
...
void FrameBufferObject::attachTextures2D(int numTextures,
		ImageTexture2D * textures, GLenum * attachment, int * mipLevel) {
	for (int i = 0; i < numTextures; ++i)
		this->attachTexture2D(
				textures[i],
				(attachment != 0) ?
						attachment[i] : (GL_COLOR_ATTACHMENT0_EXT + i),
...
# ifndef NDEBUG_GL
bool FrameBufferObject::checkValidity(std::ostream& os) {
	...
	case GL_FRAMEBUFFER_INCOMPLETE_DIMENSIONS_EXT:
		raiseGLErrorHere2(
				"Frame buffer is incomplete (GL_FRAMEBUFFER_INCOMPLETE_DIMENSIONS_EXT) ",
				_fboName.c_str());
		return false;
	case GL_FRAMEBUFFER_INCOMPLETE_FORMATS_EXT:
		raiseGLErrorHere2(
				"Frame buffer is incomplete (GL_FRAMEBUFFER_INCOMPLETE_FORMATS_EXT) ",
				_fboName.c_str());
		return false;
	case GL_FRAMEBUFFER_INCOMPLETE_DRAW_BUFFER_EXT:
		raiseGLErrorHere2(
				"Frame buffer is incomplete (GL_FRAMEBUFFER_INCOMPLETE_DRAW_BUFFER_EXT) ",
				_fboName.c_str());
		return false;
	case GL_FRAMEBUFFER_INCOMPLETE_READ_BUFFER_EXT:
		raiseGLErrorHere2(
				"Frame buffer is incomplete (GL_FRAMEBUFFER_INCOMPLETE_READ_BUFFER_EXT) ",
				_fboName.c_str());
		return false;
	case GL_FRAMEBUFFER_UNSUPPORTED_EXT:
		...
		return false;
	default:
		raiseGLErrorHere2("Frame buffer is incomplete (unknown error code) ",
				_fboName.c_str());
		return false;
	}
	return false;
...
void FrameBufferObject::disableFBORendering() {
	glBindFramebufferEXT(GL_FRAMEBUFFER_EXT, 0);
}

bool FrameBufferObject::checkBinding(char const * what) {
	GLint curFboID;
	glGetIntegerv(GL_FRAMEBUFFER_BINDING_EXT, &curFboID);
...
void checkGLErrors(char const * file, int line, ostream& os) {
	GLuint errnum;
	char const * errstr;
	bool hasError = false;
	while ((errnum = glGetError())) {
...
void checkGLErrors(char const * file, int line, string const& name,
		ostream& os) {
	GLuint errnum;
	char const * errstr;
	bool hasError = false;
...
bool checkFrameBufferStatus(char const * file, int line,
...
void raiseGLError(char const * file, int line, char const * msg,
		std::ostream& os) {
...
void ProgramBase::setProgramFromFile(char const * fileName) {
	ostringstream oss;
	...
	if (fileName[0] == '/') {
		...
		//const char* baseDir = "/home/tsou/workspace/cgCoSLAM/src/gpu/CGKLT/Shaders";
#ifdef WIN32
		const char* baseDir =
				"c:/SDKs/dlls/shaders";
#else
		const char* baseDir = "./src/tracking/CGKLT/Shaders";//"~/.slam/Shaders";
#endif
		if (baseDir == 0)
	...
	if (!is) {
		...
		cerr << "Tried " << completeName << " and " << completeName2 << "."
				<< endl;
		return;
	}
...
void Cg_ProgramBase::initializeCg() {
	if (_context == 0) {
		...
		if (cgGLIsProfileSupported(CG_PROFILE_GPU_FP))
			fragmentProfile = CG_PROFILE_GPU_FP;
		...
		if (cgGLIsProfileSupported(CG_PROFILE_GPU_VP))
			vertexProfile = CG_PROFILE_GPU_VP;
...
void Cg_ProgramBase::handleCgError() {
...
void Cg_FragmentProgram::setProgram(char const * source) {
	_source = source;
	if (_program != 0)
		cgDestroyProgram(_program);
	_program = 0;
...
void Cg_FragmentProgram::compile(char const * * compilerArgs,
		char const *entry) {
	if (_program == 0) {
		...
		if (_program == 0) {
			...
			cerr << "Cg compiler report:" << endl << cgGetLastListing(_context)
					<< endl;
		}
	}
	if (!cgIsProgramCompiled(_program))
...
void Cg_FragmentProgram::compile(std::vector<std::string> const& compilerArgs,
		char const *entry) {
...
void Cg_FragmentProgram::enable() {
	cgGLEnableProfile(fragmentProfile);
	cgGLLoadProgram(_program);
	cgGLBindProgram(_program);
}

void Cg_FragmentProgram::disable() {
	cgGLDisableProfile(fragmentProfile);
}

void Cg_FragmentProgram::parameter(char const * param, float x) {
	cgSetParameter1f(cgGetNamedParameter(_program, param), x);
}

void Cg_FragmentProgram::parameter(char const * param, float x, float y) {
...
void Cg_FragmentProgram::parameter(char const * param, int len,
		float const * array) {
	cgGLSetParameterArray1f(cgGetNamedParameter(_program, param), 0, len,
			array);
}

void Cg_FragmentProgram::matrixParameterR(char const * param, int rows,
		int cols, double const * values) {
	if (rows > 4 || cols > 4) {
		raiseGLErrorHere2("Matrix parameter should be <= 4x4.",
				_shaderName.c_str());
		return;
	}
...
void Cg_FragmentProgram::matrixParameterC(char const * param, int rows,
		int cols, double const * values) {
	if (rows > 4 || cols > 4) {
		raiseGLErrorHere2("Matrix parameter should be <= 4x4.",
				_shaderName.c_str());
...
unsigned Cg_FragmentProgram::getTexUnit(char const * param) {
...
void setupNormalizedProjection(bool flipY) {
...
void renderNormalizedQuad(GPUTextureSamplingPattern pattern, float ds,
		float dt) {
	switch (pattern) {
...
void enableTrivialTexture2DShader() {
	if (trivialTexture2DShader == 0) {
		trivialTexture2DShader = new Cg_FragmentProgram(
				"trivialTexture2DShader");
		char const * source = "void main(uniform sampler2D texture, \n"
				"                  float2 st : TEXCOORD0, \n"
...
void disableTrivialTexture2DShader() {
	if (trivialTexture2DShader)
		trivialTexture2DShader->disable();
}
