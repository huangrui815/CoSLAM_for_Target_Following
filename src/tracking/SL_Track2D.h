...
public:
	Track2DNode() :
		tkid(0), pt(), f(-1), x(0), y(0), xo(0), yo(0), pre(0), next(0) {
	}
	Track2DNode(int f1, FeaturePoint* p) :
		tkid(0), pt(p), f(f1), x(p->x), y(p->y), xo(p->xo), yo(p->yo), pre(0), next(0) {

	}
	Track2DNode(FeaturePoint* p) :
		tkid(0), pt(p), f(p->f), x(p->x), y(p->y), xo(p->xo), yo(p->yo), pre(0), next(0) {
	}
	Track2DNode(int f1, double x1, double y1, double xo1, double yo1) :
		tkid(0), pt(0), f(f1), x(x1), y(y1), xo(xo1), yo(yo1), pre(0), next(0) {
...
public:
	...
	void add(int f, double x, double y) {
		Track2DNode* node = new Track2DNode(f, x, y, 0, 0);
		add(node);
	}
	void add(int f, double x, double y, double xo, double yo) {
			Track2DNode* node = new Track2DNode(f, x, y, xo, yo);
			add(node);
	}
