...
void GPUKLT::addToFeaturePoints(int nFeatures,
	...
	for (int i = 0; i < nFeatures; i++) {
		if (features[i].status != KLT_INVALID) {
			...
			if (out[0] < 0 || out[1] < 0 || out[0] >= m_W || out[1] >= m_H){
				//m_tks[i].clear();
				continue;
			}
			FeaturePoint* p = ips.add(m_frame, m_camId, in[0], in[1], out[0], out[1]);
			if (features[i].status == KLT_TRACKED) {
				//tracked
				m_tks[i].add(p);
				trackedFeatureNum_++;
			} else {
	...
	printf("Track ratio is %f (%d)\n", trackRatio, trackedFeatureNum_);
}
void GPUKLT::addToFeaturePoints(int nFeatures,
	...
	for (int i = 0; i < nFeatures; i++) {
		if (features[i].status != KLT_INVALID) {
			...
			FeaturePoint* p = 0;
			if (features[i].fed < 0)
				p = ips.add(m_frame, m_camId, in[0], in[1], out[0], out[1]);
			else
...
void GPUKLT::reset(const ImgG& img, const Mat_f& featPts){
	assert(img.w == m_W && img.h == m_H);
	assert(featPts.m > 0 && featPts.n == 2);
	_tracker->_pyrCreator1->buildPyramidForGrayscaleImage(img.data);

	Mat_f corners(m_nMaxCorners, 3);
	for( int i = 0; i < m_nMaxCorners; i++){
		if( i < featPts.m){
			_features[i].status = 1;
			_features[i].pos[0] = featPts[2*i]/m_W;
			_features[i].pos[1] = featPts[2*i+1]/m_H;
			if( _tracker->_trackWithGain)
				_features[i].gain = 1.0f;

			corners[3*i] = _features[i].pos[0];
			corners[3*i+1] = _features[i].pos[1];
			corners[3*i+2] = 1.0f;
		}else{
			_features[i].status = -1;
			_features[i].pos[0] = -1;
...
void GPUKLT::reset(const ImgG& img,
...
void GPUKLT::readFeatPoints(Mat_d& featPts, Mat_i& flag){
	featPts.resize(m_nMaxCorners, 2);
	flag.resize(m_nMaxCorners,1);

	for( int i = 0; i < m_nMaxCorners; i++){
		flag[i] =_features[i].status;
		if( _features[i].status >= 0){
			featPts[2*i] = _features[i].pos[0] * m_W;
			featPts[2*i+1] = _features[i].pos[1] * m_H;
		}else{
			featPts[2*i] = -1;
			featPts[2*i+1] = -1;
		}
...
void GPUKLT::updateTracks(const Mat_d& pts, const Mat_i& flag) {
	assert(pts.m == flag.m && pts.m == m_nMaxCorners);
	int nOldFeatures = trackedFeatureNum_ + detectedFeatureNum_;
	...
	for (int i = 0; i < m_nMaxCorners; i++) {
		if (flag[i] >= 0) {
			...
			FeaturePoint* p = new FeaturePoint(m_frame, m_camId, in[0], in[1], out[0], out[1]);
